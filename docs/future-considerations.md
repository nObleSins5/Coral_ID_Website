...
1. ~~**Standardize which elements apply per coral anatomy**~~ **Built** (2026-07-11) — `anatomy_templates`/`anatomy_template_elements` (`sql/supabase/20_anatomy_templates.sql`), 4 templates (branching/SPS, LPS-with-corallite, tentacled LPS, polyp-based soft coral/zoanthid) assigned at the genus level to all 27 genera. `ElementColorKey` now renders the full standardized set, marking anything not yet logged as "Not yet documented" — including the case where an `element_profiles` row exists but was never given a real `color_ranges`/`color_stops` value (found live-checking Pink Stardust; fixed alongside this).
   - **TODO / revisit (2026-07-11, user request — "mark in github to revisit"):** the `element_types` codes (`corallite`, `axial_corallite`, `radial_corallite`, `polyp`, `tentacle`, `mouth_oral_disc`, `coenosarc_skin`, `base_body`, `growth_tip`, `surface_texture`) and the 4 anatomy template codes mix scientific and casual/hobbyist naming, and may not be consistent with each other or with how hobbyists actually name coral parts. Revisit the codes + `ELEMENT_LABEL` display labels (`web/components/coral-ui.tsx`) for clarity/consistency. Not urgent — display labels can change with no migration; changing `code` values needs a migration (FK'd from `element_profiles`/`anatomy_template_elements`). *(Could not open a GitHub Issue from the working session — `gh` CLI not installed — so tracked here instead.)*
2. ~~**Color-picker tool**~~ **Built** (2026-07-11), **then removed** (2026-07-12 — see the dated section at the end of this file). The crowdsourced-sample + ΔE-auto-confirm pipeline described below was retired: canonical colors now come from web research and moderator/maintainer entry only, and the sampling UI was repurposed as a personal, ephemeral comparison tool on `/identify` (never stored). Left here for history.
   - ~~Manual click-to-sample: on a morph page, a logged-in user opens the picker (`components/color-contribute-section.tsx`), clicks a point on one of that coral's community photos (or an uploaded one), tags which element it is from a dropdown scoped to the coral's anatomy template, and the region-averaged hex is read off the pixel — with a magnifier loupe and optional per-channel-sRGB white-balance correction against a tagged reference point.~~
   - ~~**Moderation model:** samples were a raw log (`element_color_samples`, `sql/supabase/21_element_color_samples.sql`), kept separate from the published `color_stops`. On submit, each sample was scored by **CIELAB ΔE76** (`web/lib/color.ts`) against the element's documented colors: within threshold on a settled taxon → auto-`confirmed`; too far out → held `out_of_range` for review; no documented color yet → `proposed`. `/moderate` had a "Coral color samples" queue.~~
3. **Anatomy guide diagrams** — not built. One reference image per template showing where each color region actually is on a real coral, labeled — still worth doing now that the templates below have been realigned.
4. **Backfill the remaining morphs still missing colors** — not built.

---

## 2026-07-12: color-ID redesign — decouple color from anatomy, retire the wiki picker, /identify comparison tool

Working from a reference zoanthid color-guide chart (a solid center + a ring of solid petal colors per named zoa) turned up two problems with the model above: crowdsourced pixel-sampling introduces noise the product doesn't want (some corals have specks/rainbows a novice can easily mis-sample, no expert gate), and requiring a user or moderator to know "skirt" vs. "radial corallite" vs. "coenosarc" before recording or comparing a color is unnecessarily technical — especially for soft-bodied corals where a zoa's face and skirt were forced into one `mouth_oral_disc` element with a 2-stop `range` pattern (rendering as a gradient blend instead of two distinct solid regions).

**Built:**
- `color_ranges` now hangs directly off `taxon_node_id` instead of through `element_profiles` (`sql/supabase/22_decouple_color_from_elements.sql`); `position_label` is an optional, suggested hint (not a required checklist item) — NULL is valid. `element_profiles` still exists, unchanged, for genuine morphology capture (shape/texture/size); it just no longer gates color.
- `polyp_soft` split into four templates that actually match their anatomy: `zoanthid_paly` (oral disc center + tentacle + up to 3 skirt colors), `mushroom_coral` (oral disc center + up to 3 skirt colors, plus a standalone `has_bubble_texture` flag for bounce/bubble mushrooms), `leather_soft_coral` (true stalk+cap growth — sarcophyton/sinularia), `mat_soft_coral` (encrusting/matting, no real stalk — briareum/xenia/clavularia). `polyp` is retired as a position everywhere (a polyp is tentacle + mouth, which can differ in color — never one element).
- The wiki color-contribution pipeline (picker UI, `element_color_samples`, ΔE auto-confirm, `/moderate`'s color-sample queue) was removed outright (`sql/supabase/23_drop_element_color_samples.sql`) — see the superseded note on item 2 above.
- The sampling UI moved to `/identify` (`web/components/photo-color-sampler.tsx`, wired into `ProposeIdentificationForm`): typing a candidate name and matching an existing coral now shows that coral's real reference color key inline (`getColorKeyForTaxon`, `app/identify/actions.ts`) plus a link to its wiki page, and an optional personal color-sampling tool against the user's own uploaded photo — purely a visual comparison, nothing is submitted or stored.
- `color_ranges.approx_percent` (nullable) was added now, ahead of the matcher below, so moderator entry can start capturing rough visual proportion without a second migration later.

**Deliberately deferred — "I see these colors + %" self-ID matcher:** a bigger, separate feature where a user enters a small set of observed colors with rough percentages (e.g. "60% blue, 30% red, 5% yellow, 5% green") and gets back candidate corals in the ballpark, matched by color set and proportion rather than requiring the user to know anatomical positions at all. Needs real design work before it's buildable:
- **Matching method**: exact hex won't work — needs perceptual color distance (CIELAB ΔE, same tool already used elsewhere in this codebase) rather than string/hex equality.
- **Percentage tolerance**: how strict a match "60/30/5/5" needs to be against a candidate's own (eventually moderator-entered) `approx_percent` values — exact-ish, banded, or just used as a secondary tiebreaker after color-set overlap.
- **Ranking/display**: how many candidates to surface, and how to present partial matches (e.g. 3 of 4 colors match) versus close-but-not-exact ones.
- Not scoped further than this — needs a product decision on the above before it's worth designing the query/UI.
