...
-- Community-contributed color samples lifted off a photo by the color-picker
-- tool (docs/future-considerations.md). A RAW LOG kept separate from the
-- published color_stops the wiki displays — a color sampled on the wrong
-- coral would otherwise pollute the documented range. status proposed ->
-- confirmed (agrees with the documented range on a settled taxon, or a
-- moderator approved) / rejected. delta_e + out_of_range are set at submit
-- time by the app's CIELAB range check (web/lib/color.ts). Full RLS +
-- rationale in sql/supabase/21_element_color_samples.sql.
CREATE TABLE element_color_samples (
    id                  uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    taxon_node_id       uuid NOT NULL REFERENCES taxon_nodes(id) ON DELETE CASCADE,
...
CREATE INDEX idx_element_color_samples_taxon
