...
2. ~~**Color-picker tool**~~ **Built** (2026-07-11). Manual click-to-sample: on a morph page, a logged-in user opens the picker (`components/color-contribute-section.tsx`), clicks a point on one of that coral's community photos (or an uploaded one), tags which element it is from a dropdown scoped to the coral's anatomy template, and the region-averaged hex is read off the pixel — with a magnifier loupe and optional per-channel-sRGB white-balance correction against a tagged reference point. Automatic region detection stays deferred.
   - **Moderation model (decided + built):** samples are a raw log (`element_color_samples`, `sql/supabase/21_element_color_samples.sql`), kept separate from the published `color_stops` so a color sampled on the wrong coral can't pollute the documented range. On submit, each sample is scored by **CIELAB ΔE76** (`web/lib/color.ts`) against the element's documented colors (seed stops + already-confirmed samples): within the threshold (`app_settings.color_sample_delta_e_threshold`, default 30) on a settled taxon → auto-`confirmed`; too far out → held `out_of_range` for review; no documented color yet → `proposed`, awaiting a moderator to bootstrap the element. `/moderate` gained a "Coral color samples" queue (approve → confirmed, reject → rejected). Confirmed samples surface in the element color key (`ElementColorKey` community swatches). **White balance stays per-channel sRGB for v1** (decision: proper CIELAB chromatic adaptation is much more work); ΔE Lab is used only for the range comparison, where it's cheap and correct.
   - **Deferred within this piece:** the picker currently lives only on the morph page (backfilling confirmed corals). Extending it to `/identify` (unconfirmed corals) needs the auto-confirm to *additionally* require the source photo's community ID to be confirmed — noted but not built. Promoting confirmed samples into the published `color_stops` (vs. just displaying them) is also deferred; the wiki reads confirmed samples directly for now. Community photos must be canvas-readable (CORS) to sample; upload always works as a fallback — verify Supabase storage CORS in a live test.
...
4. **Backfill the 36 morphs still missing elements** — not built; now unblocked (the picker from #2 exists to do the sampling with).
