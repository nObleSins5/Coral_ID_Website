...
## ✅ Shipped (2026-07-17): vocabulary sweep (clarify)

Riding along with the onboard-journey work, per the critique's F1-adjacent copy findings:

- **"Unplaced specimens" → "Not yet in the grid"** — the section heading on `/tank/[id]`, matching plainer, active phrasing consistent with the rest of the page ("Set up grid", "Reset grid"). Adjacent copy swept for consistency: the reset-grid confirm dialog, the one-shot grid-setup helper text, the quick-add slot picker's "Leave unplaced" → "Don't place yet", and three literal "Specimen" display-name fallbacks → "Unnamed coral" (matching the "Add a coral" vocabulary already used elsewhere on the same page, rather than the more clinical "specimen" — left the `Specimen` type name, `specimens` table, `/specimen/[id]` route, and component names like `AddSpecimenForm` untouched as internal identifiers, not user-facing copy).
- **Fixed the false-positive empty state**: "Everything in this tank is placed in the grid" used to fire even for a tank with zero corals at all — now three distinct, honest states: *"No corals in this tank yet — add one above"* (zero specimens), *"All your corals are placed in the grid"* (all placed), or the actual unplaced list.
- **Unified "Tier" vocabulary**: `lib/grid.ts`'s `slotLabel()` persisted `"C5 · L2"` into `grid_slots.label` at slot-creation time, while `TankGridView`'s tier-picker buttons always said "Tier 2" — two dialects for the same concept, confirmed live in the earlier critique (G2). Fixed the generator to emit `"C5 · Tier 2"`, and **backfilled all 150 already-existing stale-labeled rows** in the live Supabase project with an idempotent regex `UPDATE` (`grid_slots.label ~ ' · L[0-9]+$'` → `' · Tier N'`), so already-created tanks' slot dropdowns read correctly too, not just new ones.
- **Live-verified end-to-end** (throwaway account, a 2×2×2-tier tank): heading reads "Not yet in the grid"; zero-coral empty state reads correctly; slot dropdown reads "A1 · Tier 1" / "A1 · Tier 2" post-fix; all-placed state reads correctly once the one coral was slotted; reset-grid confirm dialog reads with the new coral-not-specimen phrasing.
- Typecheck/lint clean, 50/50 tests passing, zero detector findings.
- **Not touched**: internal identifiers (`Specimen` type, `specimens` table/columns, `/specimen/[id]` route, component names) — these aren't user-visible, and renaming them is a much larger refactor with no user-facing benefit; out of scope for a copy sweep.
